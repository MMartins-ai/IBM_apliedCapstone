...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'All Sites':
        ...
        fig = px.pie(df,values='class count',names='class',title=f'Total Success Launches for site {entered_site}')
    return fig
# TASK 4:
...
def get_scatter_chart(entered_site, payload_slider):
    ...
    #If a specific site is selected, filter the spacex_df dataframe first, then render a scatter plot to display the same as for all sites.
        ...
        fig=px.scatter(
            ...
            hover_data=['Booster Version'],
            color='Booster Version Category',
            title = 'Success by Payload Size for site '+str(entered_site))
    return fig
